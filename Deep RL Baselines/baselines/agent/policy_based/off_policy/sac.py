...
from baselines.common.network import MLPCategoricalPolicy, MLPGaussianPolicy, MLPGaussianSDEPolicy, MLPDoubleQFunction, MLPDoubleQNetwork
...
class SAC_Discrete(OffPolicyAlgorithm):
    ...
    def act(self, state, training=True):
        ...
        action = torch.multinomial(logits, 1) if training else torch.argmax(logits, dim=-1, keepdim=True)
        return action.item()
