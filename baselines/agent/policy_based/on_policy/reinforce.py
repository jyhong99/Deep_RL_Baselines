...
class REINFORCE(OnPolicyAlgorithm):
    def __init__(self, env, **config):
        super().__init__(
            env=env,
            actor_size=config.get('policy_size', (64, 64)),
            critic_size=None,
        ...
        if self.gsde_mode:
            self.policy = MLPGaussianSDEPolicy(
                self.state_dim, 
                self.action_dim, 
                self.actor_size, 
    ...
    def act(self, state, training=True, global_buffer_size=None):
        ...
        if self.gsde_mode:
            dist = self.policy.dist(state)
            action = dist.sample() if training else mu
            return torch.tanh(action + self.policy.get_noise()).cpu().numpy()
        else:
            action = torch.normal(mu, std) if training else mu
            return torch.tanh(action).cpu().numpy()

    def get_return(self, reward):
        ret = torch.clone(reward.unsqueeze(1))
        for t in reversed(range(len(ret) - 1)):
            ret[t] += self.gamma * ret[t + 1]        
        return ret

    def learn(self, states, actions, rewards, next_states=None, dones=None):
        self.policy.train()
...
class BaselineREINFORCE(OnPolicyAlgorithm):
    def __init__(self, env, **config):
        super().__init__(
            env=env,
    ...
    def learn(self, states, actions, rewards, next_states, dones):
        ...
        with torch.no_grad():
            ...
            rets = self.GAE(values, next_values, rewards, dones)[0]
        ...
        actor_loss = -(log_probs * (rets - values.detach())).mean()
