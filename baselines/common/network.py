...
class MLPMultiCategoricalPolicy(MLPBase):
    def __init__(self, state_dim, action_dims, hidden_sizes=(64, 64), activation=nn.ReLU, epsilon=1e-8):
        super(MLPMultiCategoricalPolicy, self).__init__(state_dim, hidden_sizes, activation)
        self.epsilon = epsilon
        self.output_layers = nn.ModuleList([nn.Linear(hidden_sizes[-1], action_dim) for action_dim in action_dims])
        self.apply(weights_init_)

    def forward(self, state):
        x = self.mlp(state)
        logits = [F.softmax(layer(x), dim=-1) for layer in self.output_layers]
        return logits

    def dist(self, state):
        logits = self.forward(state)
        return [Categorical(logit) for logit in logits]

    def log_prob(self, state, action):
        dists = self.dist(state)
        log_probs = torch.stack([dist.log_prob(a) for dist, a in zip(dists, action.T)], dim=1)
...
class MLPGaussianPolicy(MLPBase):
    ...
    def forward(self, state):
        x = self.mlp(state)
        mu = torch.clamp(self.mu_layer(x), MIN_MEAN, MAX_MEAN)
...
class MLPGaussianSDEPolicy(MLPBase):
    ...
    def _get_std(self, log_std):
        ...
        above_threshold = (torch.log1p(safe_log_std) + 1.) * (log_std > 0)
...
class MLPMultiQNetwork(MLPBase):
    def __init__(self, state_dim, action_dims, hidden_sizes=(64, 64), activation=nn.ReLU, dueling_mode=False):
        ...
        if dueling_mode:
            ...
            self.advantage_layers = nn.ModuleList([nn.Linear(hidden_sizes[-1], dim) for dim in action_dims])
        else:
            self.output_layers = nn.ModuleList([nn.Linear(hidden_sizes[-1], dim) for dim in action_dims])
        self.apply(weights_init_)
...
class MLPMultiDoubleQNetwork(nn.Module):
    def __init__(self, state_dim, action_dims, hidden_sizes=(64, 64), activation=nn.ReLU, dueling_mode=False):
        ...
        self.q1 = MLPMultiQNetwork(state_dim, action_dims, hidden_sizes, activation, dueling_mode)
        self.q2 = MLPMultiQNetwork(state_dim, action_dims, hidden_sizes, activation, dueling_mode)
