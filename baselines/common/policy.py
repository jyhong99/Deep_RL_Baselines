...
class OnPolicyAlgorithm:
    def __init__(self,
        env, actor_size, critic_size, actor_activation, critic_activation, update_after, 
    ...
    def GAE(self, values, next_values, rewards, dones, epsilon=1e-8):
        ...
        if self.adv_norm:
            advs = (advs - advs.mean()) / (advs.std() + epsilon)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.buffer.size >= self.update_after:
            states, actions, rewards, next_states, dones = self.buffer.sample(self.reward_norm) 
...
class OffPolicyAlgorithm:
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.prioritized_mode:
            ...
            if self.buffer.size >= self.update_after:
                states, actions, rewards, next_states, dones, weights, idxs = self.buffer.sample(self.batch_size, self.prio_beta, self.reward_norm)
            ...
            if self.buffer.size >= self.update_after:
                states, actions, rewards, next_states, dones = self.buffer.sample(self.batch_size, self.reward_norm)
                result = self.learn(states, actions, rewards, next_states, dones)
